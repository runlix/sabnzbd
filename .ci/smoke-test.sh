...
# Smoke test for SABnzbd Docker image
# This script receives IMAGE_TAG from the workflow environment

IMAGE="${IMAGE_TAG}"
PLATFORM="${PLATFORM:-linux/amd64}"
CONTAINER_NAME="sabnzbd-smoke-test-${RANDOM}"
SABNZBD_PORT="8080"
...
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}🧪 SABnzbd Smoke Test${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo "Image: ${IMAGE}"
...
cleanup() {
  ...
  if docker ps -a | grep -q "${CONTAINER_NAME}"; then
    ...
    docker logs "${CONTAINER_NAME}" > /tmp/sabnzbd-smoke-test.log 2>&1 || true
    echo "Logs saved to: /tmp/sabnzbd-smoke-test.log"
  fi
...
echo -e "${BLUE}⏳ Waiting for SABnzbd to initialize...${NC}"
...
if grep -qi "starting sabnzbd\|sabnzbd.*started" <<< "$LOGS" 2>/dev/null; then
  echo -e "${GREEN}✅ SABnzbd startup message found${NC}"
else
  echo -e "${YELLOW}⚠️  Warning: Expected startup message not found${NC}"
fi

# Check for config initialization (use grep with here-string to avoid broken pipe)
if grep -qi "config\|configuration\|initialized" <<< "$LOGS" 2>/dev/null; then
  echo -e "${GREEN}✅ Configuration initialization detected${NC}"
else
  echo -e "${YELLOW}⚠️  Warning: No configuration messages found${NC}"
fi

echo -e "${GREEN}✅ No critical errors in logs${NC}"
echo ""

# Test root endpoint with retries
echo -e "${BLUE}🌐 Testing web endpoint...${NC}"
WEB_URL="http://localhost:${SABNZBD_PORT}/"
MAX_ATTEMPTS=24
ATTEMPT=0
WEB_OK=false
...
if [ "${WEB_OK}" = true ]; then
  echo -e "${GREEN}✅ Web endpoint responding (${WEB_URL})${NC}"
else
...
API_URL="http://localhost:${SABNZBD_PORT}/api?mode=version"
if curl -fsSL --max-time 5 "${API_URL}" -o /dev/null 2>/dev/null; then
  echo -e "${GREEN}✅ API endpoint accessible (${API_URL})${NC}"
else
  echo -e "${YELLOW}⚠️  API endpoint check failed (non-critical)${NC}"
fi
echo ""
